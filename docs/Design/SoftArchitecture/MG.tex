...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucDB}:] The choice of database technology (e.g., relational database like PostgreSQL) and its core schema structure is fixed early on. 
\item[\refstepcounter{ucnum} \uctheucnum \label{ucTech}:] The foundational languages and frameworks selected for development (e.g., Node.js for backend services, React for frontend) are assumed to remain constant.

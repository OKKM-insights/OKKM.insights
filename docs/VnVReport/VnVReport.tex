...
\section{Unit Testing}
\subsection{Back-end}
Unit testing for the back-end was implemented using \texttt{pytest} and \texttt{coverage}. A variety of database connectors, core services, and API endpoints were tested to ensure correctness and robustness.

\subsubsection{Database Connector Tests}
\begin{itemize}
    \item Description: Each database connector (Projects, Labels, Labellers, ImageObjects, ImageClassMeasure) was tested for correct query execution and data conversion into internal objects.
    \item Inputs: SQL queries with known expected results
    \item Expected Outputs: Properly initialized Python data types such as \texttt{Label}, \texttt{Labeller}, or \texttt{Project}
    \item Result: Pass (100\% coverage for Label, Project, and Labeller connectors; >80\% for others)
\end{itemize}

\subsubsection{Consensus and Object Extraction Logic}
\begin{itemize}
    \item Description: Unit tests were developed to validate the consensus bounding box calculation and object extraction workflow based on user labels.
    \item Inputs: Simulated \texttt{Label} objects and image dimensions
    \item Expected Outputs: Computed bounding boxes and \texttt{ImageObject} instances
    \item Result: Pass (Core logic tested, CUDA code mocked or bypassed)
\end{itemize}

\subsubsection{API Endpoint Testing}
\begin{itemize}
    \item Description: Endpoints such as \texttt{/api/register}, \texttt{/api/login}, \texttt{/api/create\_project}, and image retrieval were tested via Flask's test client to simulate real user requests.
    \item Inputs: JSON payloads, form submissions, image uploads, and query parameters
    \item Expected Outputs: JSON responses with status codes and data matching business logic
    \item Result: Pass for most endpoints, with edge case coverage planned for future iterations
\end{itemize}

\subsubsection{Image Preprocessing and Tile Storage}
\begin{itemize}
    \item Description: The utility functions for image slicing and storing tiles in the database were tested with dummy images.
    \item Inputs: Synthetic images (numpy arrays) with dimensions both smaller and larger than the tile size
    \item Expected Outputs: Correct number of tiles and valid data entries in the database
    \item Result: Pass
\end{itemize}

\subsubsection{Report Generation}
\begin{itemize}
    \item Description: The \texttt{ReportGenerator} class was tested to ensure metrics like total labels, top labellers, and project progress are correctly computed and serialized.
    \item Inputs: Mock data simulating project, label, and labeller tables
    \item Expected Outputs: JSON report with key statistics
    \item Result: Pass after adding serialization support for custom objects
\end{itemize}

\subsubsection{Test Coverage Summary}
\begin{itemize}
    \item Description: Test coverage was tracked using \texttt{pytest-cov}. The project achieved a strong overall coverage.
    \item Key Stats: 
        \begin{itemize}
            \item Total Coverage: 76\%
            \item Files with 100\% Coverage: DataTypes, LabelDB, ProjectDB, etc.
            \item Areas for Improvement: CUDA-heavy logic in \texttt{ObjectExtractionService}, error branches in Flask endpoints
        \end{itemize}
    \item Result: 46 tests passed, 2 skipped
\end{itemize}
...
\subsection{Back-end Coverage}
The coverage results of the back-end testing can be seen in Figure~\ref{fig:FE_coverage}. Perfect coverage was not achieved, but we believe our unit tests supplemented with our manual and usability tests provide sufficient coverage of the code.
1. Strong Overall Coverage \\
...
Multiple database connectors (Project, Label, Labeller, etc.) are covered and validated.
...
4. High Coverage on Utility and Interface Code\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{BE_coverage.png}
    \caption{Back-end Testing Coverage Results}
    \label{fig:BE_coverage}
\end{figure}

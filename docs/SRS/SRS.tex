...
\midrule
03/08/2025 & Kyle & Revision 1.0\\
04/01/2025 & Kyle & Revision 1.1\\
\bottomrule
\end{tabularx}
...
\begin{itemize}
    \item \textbf{Labellers Have Reliable Internet Access}: It is assumed that all users participating in labeling have stable and sufficient internet connectivity to access the platform, label data, and interact with the system without major disruptions.
    \item \textbf{Positive User Experience Will Attract and Retain Labellers}: The platform will rely on the assumption that a smooth user interface, fair compensation, and clear instructions will be enough to attract a substantial user base of labellers and keep them engaged over time.
...
\begin{tabular}{ |c|c|c| } % chktex 44
  \hline
    ...
    & & \\
    \hline
    Labeller &  LabellerID & Class \\ 
    \hline
    Customer &  CustomerID & Class \\ 
    \hline
    Labeller Payment Details &  LabellerID & Class \\ 
    &  Payment Details & \\
    \hline
    Customer Payment Details &  CustomerID & Class \\ 
    &  Payment Details & \\
    \hline
    Labeller Personal Details &  LabellerID & Class \\ 
    &  Personal Details & \\
    \hline
    Customer Personal Details &  CustomerID & Class \\ 
    &  Personal Details & \\
    \hline
    Historic Accuracy &  LabellerID & Class \\ 
    &  Object Class & \\
    &  Accuracy & \\
    &  Samples & \\
    \hline
...
\begin{longtable}
 {p{0.1\textwidth} | p{0.35\textwidth} | p{0.2\textwidth} | p{0.35\textwidth}}
  ...
  4 & Update User Information & Labellers, Customers & Information to change (in),  Updated information (out)\\
   \midrule
  5 & Annotate Images & Labellers & Image \& annotation (in), Labeled image (out)\\
  \midrule
  6 & Segment Annotation & Labellers & Segmented image \& user selections (in), Image with labeled segments (out)\\
  \midrule
  7 & Bounding Box Annotation & Labellers & Image \& box annotations (in), Image with labeled boxes (out)\\
  \midrule
  8 & Classification & Labellers & Image \& choices (in), Image labeled with the selected choice (out)\\
  \midrule
...
\textbf{Success Scenario:}
\begin{enumerate}
    \item A satellite image related to the selected project and a description of what to annotate is shown to the labeller
    \item The labeller annotates the satellite image to the best of their ability
    \item The annotated satellite image is submitted by the labeller
    \item The labeller is rewarded and the satellite image is stored by the system
    ...
    \item The next satellite image of the selected project is shown to the labeller and the process repeats
\end{enumerate}
\textbf{Post-condition:} All submitted annotated satellite images are stored by the system. The labeller's reward balance is updated to reflect their submissions\\
...
\textbf{Success Scenario:}
\begin{enumerate}
    \item The same as Annotate Images, but the labeller is given a satellite image segmented into different parts and must label each segment to the best of their ability 
\end{enumerate}
\textbf{Post-condition:} All submitted annotated satellite images are stored by the system. The labeller's reward balance is updated to reflect their submissions\\
\textbf{Related:} \hyperref[sec:FR13]{FR13}
...
\textbf{Success Scenario:}
\begin{enumerate}
    \item The same as Annotate Images, but the labeller must annotate areas of the satellite image by drawing boxes and providing a label for each box
\end{enumerate}
\textbf{Post-condition:} All submitted annotated satellite images are stored by the system. The labeller's reward balance is updated to reflect their submissions\\
\textbf{Related:} \hyperref[sec:FR13]{FR13}
...
\textbf{Success Scenario:}
\begin{enumerate}
    \item The same as Annotate Images, but the labeller must classify what category a satellite image belongs to given several options
\end{enumerate}
\textbf{Post-condition:} All submitted annotated satellite images are stored by the system. The labeller's reward balance is updated to reflect their submissions\\
\textbf{Related:} \hyperref[sec:FR13]{FR13}
...
\begin{enumerate}
    ...
    \item The system asks the customer to provide sample labeled images that can be shared with labellers
    ...
    \item The system makes the project available to labellers so they can annotate the satellite images
\end{enumerate}
\textbf{Post-condition:} The image analysis project has been created and labellers can start annotating the satellite images\\
...
\begin{itemize}
  \item \textbf{Priority: High}
  \item \textbf{Description:} The system shall allow Labellers to create a new user account.
  \item \textbf{Rationale:} The Labellers must have an account to request the services provided by the system.
  \item \textbf{Fit Criterion:} Labeller is created in and stored in the database.
...
\begin{itemize}
  \item \textbf{Priority: High}
  \item \textbf{Description:} The system shall allow Labellers with an existing account to authenticate themselves using the credentials stored by the system.
  \item \textbf{Rationale:} In order to access privileged information, the system must Labellers customers.
\item \textbf{Fit Criterion:} Labellers can use correct credentials to log in to user account.
...
\begin{itemize}
  \item \textbf{Priority: Medium}
  \item \textbf{Description:} The system shall allow authenticated Labellers to edit their account information.
  \item \textbf{Rationale:} After creating an account, the Labellers must have the necessary tools to modify the information they have provided.
  \item \textbf{Fit Criterion:} After logging in, Labellers can modify the stored personal information.
...
\begin{itemize}
  \item \textbf{Priority: Out of Scope}
  \item \textbf{Description:} The system shall allow Labellers to request their accrued earnings be transferred to their provided banking platform.
  \item \textbf{Rationale:} The system must ensure Labellers are compensated for their efforts.
\item \textbf{Fit Criterion:} After logging in, Labellers can request their earnings be deposited in the account stored in their personal information.
...
\begin{itemize}
  \item \textbf{Priority: High}
  \item \textbf{Description:} The system shall allow Labellers to annotate images displayed to them with the purpose of collecting annotation data.
  \item \textbf{Rationale:} This is the key service the system provides.
  \item \textbf{Fit Criterion:} Labellers will be able to annotate images.
...
\begin{itemize}
  \item \textbf{Priority: High}
  \item \textbf{Description:} The system shall combine annotations obtained from authenticated Labellers into a consolidated report.
  \item \textbf{Rationale:} This is the key service the system provides.
...
\begin{itemize}
  \item \textbf{Priority: Medium}
  \item \textbf{Description:} The system shall process new user requests within 15 minutes of a Labeller  completing the account creation process 90\% of the time, and within 48 hours in all cases. 
  \item \textbf{Rationale:} To improve engagement from Labellers, there should be minimal delay in getting started.
  \item \textbf{Fit Criterion:}\\ Let $t_{\text{newLabelAccount}}$ be the time it takes to process a new Labeller account request, in hours.\\ \probP($t_{\text{newLabelAccount}} < 0.25) \geq 0.9 \wedge \probP(t_{\text{newLabelAccount}} < 48) = 1 $
...
\begin{itemize}
  \item \textbf{Priority: High}
  \item \textbf{Description:} The system shall take no longer than 10 seconds to display the next image to be labeled to a labeller, if there is one available.
  \item \textbf{Rationale:} To reduce friction for labellers, the system must ensure there is no unnecessary delay in preparing the next job. For users exposed to modern media, attention spans can be expected to be less than 10 seconds [7].
...
\begin{itemize}
  \item \textbf{Priority: Out of Scope}
  \item \textbf{Description:} The system shall deliver earned payouts to labellers within 7 business days of a request being made through the system.
  \item \textbf{Rationale:} The labellers are entitled to their earned income, and there must not be unnecessary delay. 7 days accounts for delays in the platform used to distribute payments.
...
\begin{itemize}
  ...
  \item \textbf{Rationale:} Average data label accuracy for competitors is greater than 75\% [8]. The system should at a minimum provide the same label accuracy.
  ...
  Let $L_{\text{Guess}}: O \rightarrow C $ be the function derived from the system which maps objects in $O$ to their assumed classes in $C$.
...
\begin{itemize}
  \item \textbf{Priority: Low}
  \item \textbf{Description:} The system shall have the capacity to support enough labellers to meet all service request deadlines.
...
\begin{itemize}
  ...
  \item \textbf{Fit Criterion:} Testing results in fewer than 10 critical bugs identified and includes actionable feedback from at least 50 labellers, leading to necessary iterative improvements.
\end{itemize}
\subsubsection*{NFR-OE9}
\label{sec:OE9}
\begin{itemize}
  \item \textbf{Priority: Out of Scope}
  \item \textbf{Description:} Each release shall preserve the core functionality of existing features without introducing regressions or deprecation.
...
\begin{itemize}
  \item \textbf{Priority: Medium}
  \item \textbf{Description:} All maintenance required for the system shall be possible to complete by a competent software developer after reading all the documentation provided in the source repository.
...
\begin{itemize}
  ...
  \item \textbf{Fit Criterion:} Upon user disconnect and reconnect, the system will not accept any updates from the user not submitted before the disconnect occurred.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Rationale:} Legal requirement to operate.
  \item \textbf{Fit Criterion:} All relevant tax codes must be satisfied when accepting payment from Customers or paying out earnings to Labellers. Specifically \href{https://www.ontario.ca/laws/statute/90c40}{Ontario Tax Act, 1990} and \href{https://laws-lois.justice.gc.ca/eng/acts/i-3.3/}{Income Tax Act 1985}
\end{itemize}
\subsubsection*{NFR-CO2}
\label{sec:CO2}
\begin{itemize}
  \item \textbf{Priority: Low}
  \item \textbf{Description:} The system shall allow Customers to restrict Labellers from certain regions from labeling the images related to their service request.
  \item \textbf{Rationale:} Customers may be uploading sensitive images, which are inappropriate for international Labellers to view.
...
\section{Open Issues}
\textbf{Task Assignment Algorithm: }To ensure labellers are engaged as they complete tasks and to obtain the highest quality of information possible, the system must implement an intelligent task allocation system. This system has not yet been determined.
...
\begin{itemize} 
    ...
    \item \textbf{Frontend Development (23-Nov-2024 to 05-Jan-2025)}: Design and implement UI/UX features, integrate backend APIs, and ensure an interactive user experience for both clients and labellers. Aim for a fully functional frontend that complements backend capabilities.
...
\begin{enumerate}
  \item Investing time into UI/UX design to ensure the platform is intuitive and easy to use for both data labellers and end clients. This includes designing workflows, views, and ensuring responsive design across devices.
  \end{enumerate}
...
\begin{enumerate}
  \item Ongoing maintenance of the platform to ensure uptime, implement updates, and address technical issues. This also includes providing customer support to both data labellers and end clients.
  \end{enumerate}
...
\begin{enumerate}
  \item Marketing efforts to attract both data labellers and end clients to the platform. This includes digital marketing campaigns, partnerships with relevant organizations, and participation in industry events.
  \end{enumerate}
...
\begin{itemize}
  \item Improvements to batch upload of images. It is likely at some point in the future of the project, users will prefer to upload multiple images to one project, and be able to view progress for each image from a single UI. This will require additional requirements to describe functionality.
  \item Payment support. When launched to customers, the site will have to support payment. This will require modifying the priority of existing requirements related to payments.
  \item Gamification of labelling. Currently, the labeller experience is a secondary priority in the project. To improve the experience for labellers, we may add different types of labeling tasks, rewards, or additional elements to increase engagement.
\end{itemize}
\section{Ideas for Solution}

\begin{itemize} 
    \item \textbf{AI-Driven Labeling System}: Implement a machine learning model for preliminary image analysis to assist human labellers. This AI system can automatically pre-label sections of satellite images, allowing human labellers to validate or adjust these labels, significantly speeding up the process.
    ...
    \item \textbf{Real-Time Collaboration and Consensus Model}: Allow multiple labellers to work on the same image in parallel, using a consensus model to determine the most accurate labels. Incorporate a review mechanism to resolve discrepancies and ensure that the final labeled data meets the required quality standards.
    
    \item \textbf{Automated Quality Assurance (QA) and Feedback}: Develop a QA system that automatically checks the quality of labeled data by comparing it to known benchmarks or running cross-validation with multiple labellers. Provide labellers with instant feedback to help improve their accuracy over time.
    ...
    \item \textbf{Flexible API for Client and User Integration}: Create an API that allows clients to easily upload satellite imagery, retrieve labeled data, and interact with the system programmatically. Also, provide endpoints for labellers to claim tasks, submit work, and check their progress or earnings.
    ...
    \item \textbf{Secure Payment and Compensation System}: Implement a secure and transparent payment system that allows clients to pay for labeled data and compensates labellers based on their contributions. Use automated transaction management to ensure timely and accurate payments to all users.
...
\begin{enumerate}
    ...
    \item M Khaled. (2024). \textit{The Attention Span Crisis in the Digital Age Explained—With Recent Statistics for 2024}. https://profiletree.com/attention-span-crisis-digital-age-statistics/
    \item Hsueh, P.-Y \& Melville, Prem \& Sindhwani, Vikas. (2009). \textit{Data quality from crowdsourcing: A study of annotation selection criteria}. Proceedings of the NAACL HLT 2009 Workshop on Active Learning for Natural Language Processing. 27-35. 
  \end{enumerate}
